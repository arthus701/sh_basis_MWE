// This c++ code is an adapted version from pymagglobals dsh_basis function
//
    ...
    for (int i=0; i < I; i++) {
        ...
        for (int l=1; l <= lmax; l++) {
            if (internal == true) {
                ...
                lfac = -(l+1);
                // Scaling
                scl = std::pow((R / (zptr[i*3+2]*1.)), l+2);
            } else {
                // l-factor
                lfac = l;
                // Scaling
                scl = std::pow(((zptr[i*3+2]*1.) / R), l-1);
            }
            for (int m=0; m <= std::min(l, mmax); m++) {
                ind = lm2i(l, m);
                ...
                */
                ...
                if (m != 0) {
                    ...
                    // Down component
                    rptr[(ind+1)*I*3 + i*3+2] *= lfac*sin_p;
